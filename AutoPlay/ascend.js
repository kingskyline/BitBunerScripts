...
const argsSchema = [
    ...
    ['allow-soft-reset', false], // If set to true, allows AutoPlay/ascend.js to invoke a **soft** reset (installs no augs) when no augs are affordable. This is useful e.g. when ascending rapidly to grind hacknet hash upgrades.
    ...
    ['on-reset-script', null], // By default, will start with `stanek.js` if you have stanek's gift, otherwise `AutoPlay/daemon.js`.
...
export async function main(ns) {
    ...
    ns.run(getFilePath('AutoPlay/spend-hacknet-hashes.js'), 1, '--liquidate');
    ...
    if (hasTixApiAccess) {
        ...
        do {
            ...
            pid = ns.run(getFilePath('/stocks/stockmaster.js'), 1, '--liquidate');
            if (pid) await waitForProcessToComplete(ns, pid, true);
            else log(ns, `ERROR: Failed to run "/stocks/stockmaster.js --liquidate" to sell ${ownedStocks} owned stocks. Will try again soon...`, false, 'true');
            await ns.sleep(1000);
    ...
    if (options['skip-staneks-gift']) {
        log(ns, 'INFO: Sending the --ignore-stanek argument to AutoPlay/faction-manager.js')
        facmanArgs.push('--ignore-stanek');
    }
    pid = ns.run(getFilePath('AutoPlay/faction-manager.js'), 1, ...facmanArgs);
    ...
    // STEP 5: Try to Buy 4S data / API if we haven't already and can afford it (although generally /stocks/stockmaster.js would have bought these if it could)
    ...
    if (10 in dictSourceFiles) {
        ...
        ns.run(getFilePath('AutoPlay/sleeve.js'), 1, '--reserve', '0', '--aug-budget', '1', '--min-aug-batch', '1', '--buy-cooldown', '0', '--disable-training');
    ...
    if (2 in dictSourceFiles) {
        ...
        ns.run(getFilePath('AutoPlay/gangs.js'), 1, '--reserve', '0', '--augmentations-budget', '1', '--equipment-budget', '1');
        await ns.sleep(500); // Give it time to make its initial purchases. Note that we do not block on the process shutting down - it will keep running.
    }
    ...
    */
    ...
    await waitForProcessToComplete(ns, ns.run(getFilePath('AutoPlay/cleanup.js')), true);
    ...
    if (options.reset || options['install-augmentations']) {
        ...
        const resetScript = options['on-reset-script'] ??
            // Default script (if none is specified) is stanek.js if we have it (which in turn will spawn AutoPlay/daemon.js when done)
            (purchasedAugmentations.includes(`Stanek's Gift - Genesis`) ? getFilePath('stanek.js') : getFilePath('AutoPlay/daemon.js'));
        if (noAugsToInstall)
