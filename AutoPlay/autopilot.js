...
const argsSchema = [ // The set of all command line arguments
    ...
    ['disable-bladeburner', false], // This will instruct AutoPlay/daemon.js not to run the AutoPlay/bladeburner.js, even if bladeburner is available.
    ...
    ['disable-casino', true], // Set to true to disable running the AutoPlay/casino.js script automatically
    ...
    ['xp-mode-interval-minutes', 55], // Every time this many minutes has elapsed, toggle AutoPlay/daemon.js to runing in --xp-only mode, which prioritizes earning hack-exp rather than money
    ['xp-mode-duration-minutes', 5], // The number of minutes to keep AutoPlay/daemon.js in --xp-only mode before switching back to normal money-earning mode.
...
export async function main(ns) {
    ...
    const defaultBnOrder = [ // The order in which we intend to play bitnodes
        ...
        10.1, // Hard.   Unlock Sleeves (which tremendously speed along gangs outside of BN2) and grafting (can speed up slow rep-gain BNs). // TODO: Buying / upgrading sleeve mem has no API, requires manual interaction. Can we automate this with UI clicking like AutoPlay/casino.js?
        ...
        14.2, // Hard.   Boosts AutoPlay/go.js bonuses, but note that we can automate IPvGO from the very start (BN1.1), no need to unlock it. 14.1 doubles all bonuses. 14.2 unlocks the cheat API.
        ...
        14.3, // Hard.   Makes AutoPlay/go.js cheats slightly more successful, increases max go favour from (100->120) and not too difficult to get out of the way
    ...
    async function main_start(ns) {
        ...
        while (keepRunning) {
            try {
                ...
                log(ns, `WARNING: AutoPlay/autopilot.js Caught (and suppressed) an unexpected error:` +
    ...
    async function mainLoop(ns) {
        ...
        return shouldWeKeepRunning(ns); // Return false to shut down AutoPlay/autopilot.js if we installed augs, or don't have enough home RAM
    }
    ...
    async function checkOnDaedalusStatus(ns, player, stocksValue) {
        ...
        if (player.factions.includes("Daedalus")) {
            ...
            // If we previously took any action to "rush" Daedalus, keep the momentum going by restarting AutoPlay/work-for-factions.js
            ...
            if (prioritizeHackForDaedalus || reservingMoneyForDaedalus) {
                ...
                if (prioritizeHackForDaedalus) {
                    prioritizeHackForDaedalus = false; // Can turn off this flag now so AutoPlay/daemon.js can be reverted
                ...
                log(ns, `SUCCESS: We sped along joining the faction 'Daedalus' ${reason}. ` + // Pat ourselves on the back
                    `Restarting AutoPlay/work-for-factions.js to speed along earn rep.`, false, 'success');
                killScripts.push("AutoPlay/work-for-factions.js"); // Schedule this to be killed (will be restarted) on the next script loop.
        ...
        if (player.skills.hacking < 2500) {
            ...
            // set a flag to switch AutoPlay/daemon.js into --xp-only mode, to prioritize earning hack exp over money
        ...
        if (player.money < moneyReq && totalWorth > moneyReq * 1.001 /* slight buffer to account for timing issues */) {
            ...
            if (player.money < moneyReq) { // Only liquidate stocks if we don't have enough cash lying around.
                ...
                launchScriptHelper(ns, '/stocks/stockmaster.js', ['--liquidate']);
    ...
    async function checkIfBnIsComplete(ns, player) {
        ...
        // for AutoPlay/daemon.js to get a little money, buy the crack tools, and nuke the server first.
        if (bnComplete) {
        ...
        let pid = launchScriptHelper(ns, 'AutoPlay/cleanup.js');
        if (pid) await waitForProcessToComplete(ns, pid);

        // In all likelihood, AutoPlay/daemon.js has already nuked this like it does all servers, but in case it hasn't:
        pid = launchScriptHelper(ns, 'AutoPlay/Tasks/crack-host.js', ['w0r1d_d43m0n']);
        if (pid) await waitForProcessToComplete(ns, pid);
    ...
    async function checkOnRunningScripts(ns, player) {
        ...
        if (!findScript('/stocks/stockmaster.js') && !disableStockmasterForDaedalus && homeRam >= 32)
            launchScriptHelper(ns, '/stocks/stockmaster.js', [
        ...
        if ((10 in unlockedSFs) && (2 in unlockedSFs) && !findScript('AutoPlay/sleeve.js')) {
            let sleeveArgs = [];
            ...
            launchScriptHelper(ns, 'AutoPlay/sleeve.js', sleeveArgs);
        }

        // Spend hacknet hashes on our boosting best hack-income server once established
        let existingSpendHashesProc = findScript('AutoPlay/spend-hacknet-hashes.js', s => s.args.includes("--spend-on-server"))
                ...
                if (bestServer && (gain > spendHashesMultThreshold || resetInfo.currentNode == 9)) {
                    ...
                    if (existingSpendHashesProc) {
                        ...
                        if (currentBoostTarget != bestServer || isReducingSecurity != shouldReduceMinSecurity) {
                            log(ns, `Killing a prior AutoPlay/spend-hacknet-hashes.js process targetting ${currentBoostTarget} because ` +
                                (currentBoostTarget != bestServer ? `The new best income server is ${bestServer}.` : 'We no longer need to reduce minimum security.'), true);
                            await killScript(ns, 'AutoPlay/spend-hacknet-hashes.js', null, existingSpendHashesProc);
                    ...
                    if (!existingSpendHashesProc) { // 
                        ...
                        launchScriptHelper(ns, 'AutoPlay/spend-hacknet-hashes.js', spendHashesArgs);
                    }
                } else if (gain <= 1)
                    log_once(ns, `INFO: The best server (${bestServer})'s hack income multiplier (${formatNumber(gain)}) is currently too severely penalized ` +
                        `(< ${spendHashesMultThreshold}) to merit launching AutoPlay/spend-hacknet-hashes.js to boost servers. (Configure with --spend-hashes-on-server-hacking-threshold)`);
                else
                    log(ns, `WARNING: strServerIncomeInfo was not empty, but could not determine best server:\n${strServerIncomeInfo}`);
            }
        }

        const existingDaemon = findScript('AutoPlay/daemon.js');
        ...
        // If AutoPlay/daemon.js is already running in --looping-mode, we should not restart it, because
        // TODO: currently AutoPlay/daemon.js has no ability to kill it's loops on shutdown (so the next instance will be stuck with no RAM available)
        if (existingDaemon?.args.includes("--looping-mode"))
            daemonArgs = existingDaemon.args;
        else {
            // Determine the arguments we want to run AutoPlay/daemon.js with. We will either pass these directly, or through stanek.js if we're running it first.
            ...
            // expensive / laggy. To help with this, AutoPlay/daemon.js supports "looping mode", to just spawn one long-lived script that does H/G/W in a loop.
            if (false /* TODO: LOOPING MODE DISABLED UNTIL WORKING BETTER */ && player.skills.hacking >= hackThreshold) {
                ...
                // Log a special notice if we're going to be relaunching AutoPlay/daemon.js for this reason
                if (!existingDaemon || !(existingDaemon.args.includes("--looping-mode")))
                    daemonRelaunchMessage = `Hack level (${player.skills.hacking}) is >= ${hackThreshold} (--high-hack-threshold): Starting AutoPlay/daemon.js in high-performance hacking mode.`;
            ...
            } else { // XP-ONLY MODE: We can shift AutoPlay/daemon.js to this when we want to prioritize earning hack exp rather than money
                ...
                let useXpOnlyMode = prioritizeHackForDaedalus || prioritizeHackForWd ||
                    // In BNs that give no money for hacking, always start AutoPlay/daemon.js in this mode (except BN8, because TODO: --xp-only doesn't handle stock manipulation)
                ...
                if (!useXpOnlyMode) { // Otherwise, respect the configured interval / duration
                    ...
                    // If AutoPlay/daemon.js was previously running in hack exp mode, prepare a message indicating that we 're switching back
                    else if (existingDaemon?.args.includes("--xp-only"))
                        daemonRelaunchMessage = `Time is up for "xp-mode", Relaunching AutoPlay/daemon.js normally to focus on earning money for ${xpInterval} minutes (--xp-mode-interval-minutes)`;
                }
                if (useXpOnlyMode) {
                    daemonArgs.push("--xp-only", "--silent-misfires", "--no-share");
                    // If AutoPlay/daemon.js isn't already running in hack exp mode, prepare a message to communicate the change
                    if (!existingDaemon?.args.includes("--xp-only"))
                        daemonRelaunchMessage = prioritizeHackForWd ? `We're close to the required hack level destroy the BN.` :
                            prioritizeHackForDaedalus ? `Hack Level is the only missing requirement for Daedalus, so we will run AutoPlay/daemon.js in --xp-only mode to try and speed along the invite.` :
                                (bitNodeMults.ScriptHackMoney * bitNodeMults.ScriptHackMoneyGain == 0) ?
                                    `The current BitNode does not give any money from hacking, so we will run AutoPlay/daemon.js in --xp-only mode.` :
                                    `Relaunching AutoPlay/daemon.js to focus on earning Hack Experience for ${options['xp-mode-duration-minutes']} minutes (--xp-mode-duration-minutes)`;
                }
            }
            // Prevent daemon from starting "work-for-faction.js" since we now manage that script
            daemonArgs.push('--disable-script', getFilePath('AutoPlay/work-for-factions.js'));
            ...
            if (options['disable-bladeburner']) daemonArgs.push('--disable-script', getFilePath('AutoPlay/bladeburner.js'));
            // Relay the option to suppress tail windows
            if (options['no-tail-windows']) daemonArgs.push('--no-tail-windows');
            // If we have SF4, but not level 3, instruct AutoPlay/daemon.js to reserve additional home RAM
            if ((4 in unlockedSFs) && unlockedSFs[4] < 3)
                daemonArgs.push('--reserved-ram', 32 * ((unlockedSFs[4] ?? 0) == 2 ? 4 : 16));
        }

        // Once stanek's gift is accepted, launch it once per reset before we launch daemon (Note: stanek's gift is auto-purchased by AutoPlay/faction-manager.js on your first install)
        ...
        if ((13 in unlockedSFs) && !stanekLaunched && !stanekRunning && installedAugmentations.includes(augStanek)) {
            ...
            const stanekArgs = ["--on-completion-script", getFilePath('AutoPlay/daemon.js')]
            if (options['no-tail-windows']) stanekArgs.push('--no-tail'); // Relay the option to suppress tail windows
            if (daemonArgs.length >= 0) stanekArgs.push("--on-completion-script-args", JSON.stringify(daemonArgs)); // Pass in all the args we wanted to run AutoPlay/daemon.js with
        ...
        if (launchDaemon) {
            if (existingDaemon) {
                daemonRelaunchMessage ??= `Relaunching AutoPlay/daemon.js with new arguments since the current instance doesn't include all the args we want.`;
                log(ns, daemonRelaunchMessage);
            }
            let daemonPid = launchScriptHelper(ns, 'AutoPlay/daemon.js', daemonArgs);
        ...
        // The following args are ideal when running 'AutoPlay/work-for-factions.js' to rush unlocking gangs (earn karma)
        ...
        // If gangs are unlocked, micro-manage how 'AutoPlay/work-for-factions.js' is running by killing off unwanted instances
        if (2 in unlockedSFs) {
            ...
            // Detect if a 'AutoPlay/work-for-factions.js' instance is running with args that don't match our goal. We aren't too picky,
            // (so the player can run with custom args), but should have --crime-focus if (and only if) we're still working towards a gang.
            const wrongWork = findScript('AutoPlay/work-for-factions.js', !rushGang ? s => s.args.includes("--crime-focus") :
            ...
            if (wrongWork) await killScript(ns, 'AutoPlay/work-for-factions.js', null, wrongWork);
            ...
            if (playerInGang && !findScript('AutoPlay/gangs.js'))
                launchScriptHelper(ns, 'AutoPlay/gangs.js');
        }

        // Launch work-for-factions if it isn't already running (rules for maybe killing unproductive instances are above)
        // Note: We delay launching our own 'AutoPlay/work-for-factions.js' until daemon has warmed up, so we don't steal it's "kickstartHackXp" study focus
        if ((4 in unlockedSFs) && !findScript('AutoPlay/work-for-factions.js') && Date.now() - daemonStartTime > 30000) {
            ...
            launchScriptHelper(ns, 'AutoPlay/work-for-factions.js', rushGang ? rushGangsArgs : workForFactionsArgs);
        }
    }
    ...
    async function maybeAcceptStaneksGift(ns, player) {
        ...
        if (await getNsDataThroughFile(ns, 'ns.stanek.acceptGift()')) {
            ...
            log(ns, `WARNING: AutoPlay/autopilot.js tried to accepted Stanek's Gift, but was denied.`, true, 'warning');
    ...
    async function maybeDoCasino(ns, player) {
        ...
        if (casinoEarnings >= 1e10) {
            log(ns, `INFO: Skipping running AutoPlay/casino.js, as we've previously earned ${formatMoney(casinoEarnings)} and been kicked out.`);
            return ranCasino = true;
        }
        // If we already have more than 1t money but hadn't run AutoPlay/casino.js yet, don't bother. Another 10b won't move the needle much.
        const playerWealth = player.money + (await getStocksValue(ns));
        if (playerWealth >= 1e12) {
            log(ns, `INFO: Skipping running AutoPlay/casino.js, since we're already ridiculously wealthy (${formatMoney(playerWealth)} > 1t).`);
            return ranCasino = true;
        }
        ...
        if (resetInfo.currentNode != 8) {
            ...
            if (getTimeInAug() < 60000)
                return log_once(ns, `INFO: Waiting a minute to establish player income before deciding whether AutoPlay/casino.js is needed.`);
            ...
            if (incomePerMinute >= 5e9) {
                log(ns, `INFO: Skipping running AutoPlay/casino.js this augmentation, since our income (${formatMoney(incomePerMinute)}/min) >= 5b/min`);
                return ranCasino = true;
            }
        }
        ...
        if (player.city != "Aevum" && player.money < 300000)
            return log_once(ns, `INFO: Waiting until we have ${formatMoney(300000)} to travel to Aevum and run AutoPlay/casino.js`);

        // Run AutoPlay/casino.js (and expect this script to get killed in the process)
        // Make sure "AutoPlay/work-for-factions.js" is dead first, lest it steal focus and break the casino script before it has a chance to kill all scripts.
        await killScript(ns, 'AutoPlay/work-for-factions.js');
        await killScript(ns, 'AutoPlay/daemon.js'); // We also have to kill daemon which can make us study.
        ...
        if (4 in unlockedSFs) // No big deal if we can't, AutoPlay/casino.js has logic to find the stop button and click it.
            _ = await getNsDataThroughFile(ns, `ns.singularity.stopAction()`);

        const pid = launchScriptHelper(ns, 'AutoPlay/casino.js', ['--kill-all-scripts', true, '--on-completion-script', ns.getScriptName()]);
        if (pid) {
            await waitForProcessToComplete(ns, pid);
            await ns.sleep(10000); // Give time for this script to be killed if the game is being restarted by AutoPlay/casino.js
            // Otherwise, something went wrong
            log(ns, `ERROR: Something went wrong. AutoPlay/casino.js was run, but we haven't been killed. It must have run into a problem...`)
        }
    }
    ...
    async function maybeInstallAugmentations(ns, player) {
        ...
        if (reservedPurchase && installCountdown <= Date.now()) {
            log(ns, "INFO: Manually running AutoPlay/faction-manager.js to ensure previously reserved purchase is still obtainable.");
            ns.write(factionManagerOutputFile, "", "w"); // Reset the output file to ensure it isn't stale
            const pid = launchScriptHelper(ns, 'AutoPlay/faction-manager.js');
        ...
        if (!shouldReset) {
            setStatus(ns, `Currently at ${formatDuration(getTimeInAug())} since last aug. ` +
                ...
                ` (\`run AutoPlay/faction-manager.js\` for details)`, augSummary);
        ...
        const resetLog = `  Invoking AutoPlay/ascend.js at ${formatDuration(getTimeInAug()).padEnd(11)} since last aug to install: ${augSummary}`;
        persist_log(ns, log(ns, resetLog, true, 'info'));

        // Kick off AutoPlay/ascend.js
        let errLog;
        ...
        let pid = launchScriptHelper(ns, 'AutoPlay/ascend.js', ascendArgs);
        if (pid) {
            ...
            errLog = `ERROR: AutoPlay/ascend.js ran, but we're still here. Something must have gone wrong. Will try again later`;
        } else
            errLog = `ERROR: Failed to launch AutoPlay/ascend.js (pid == 0). Will try again later`;
    ...
    */
    ...
    async function checkIfGrafting(ns) {
        ...
        if (currentWork?.type == "GRAFTING") {
            if (!wasGrafting) // Only log the first time we detect we've started grafting
                log(ns, "Grafting in progress. AutoPlay/autopilot.js will make sure to not install augmentations or otherwise interrupt it.");
    ...
    function manageReservedMoney(ns, player, stocksValue) {
        ...
        // - AutoPlay/casino.js ignores the reserve, so the above takes care of ensuring our casino seed money isn't spent
    ...
    /** Logic to determine whether we should keep running, or shut down AutoPlay/autopilot.js for some reason.
     * @param {NS} ns
    ...
    function shouldWeKeepRunning(ns) {
        ...
        // If we've gotten AutoPlay/daemon.js launched, but only have 8GB ram, we must shut down for now
        if (homeRam == 8 && daemonStartTime > 0) {
            log(ns, `WARN: (not an actual warning, just trying to make this message stand out.)` +
                ...
                `\n\n  Currently, your available RAM on home (8 GB) is too small to keep AutoPlay/autopilot.js running.` +
                `\n  The priority should just be to run "AutoPlay/daemon.js" for a while until you have enough money to` +
                ...
                `\n\n` + '-'.repeat(100), true);
            return false; // AutoPlay/daemon.js needs more room to breath
        }
