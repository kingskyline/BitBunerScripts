...
const argsSchema = [
    ...
    ['disable-spending-hashes', false], // Set to true to not spawn AutoPlay/spend-hacknet-hashes.js to spend hashes on bladeburner
];
...
export async function main(ns) {
    ...
    while (true) {
        try { await mainLoop(ns); }
            ...
            log(ns, `WARNING: AutoPlay/bladeburner.js Caught (and suppressed) an unexpected error in the main loop:\n` +
...
async function beingInBladeburner(ns) {
    ...
    while (!(await getNsDataThroughFile(ns, 'ns.bladeburner.inBladeburner()'))) {
        try {
            if (player.skills.strength < 100 || player.skills.defense < 100 || player.skills.dexterity < 100 || player.skills.agility < 100)
                ...
                if (9 in ownedSourceFiles && options['disable-spending-hashes'])
                    message += ' --disable-spending-hashes is set, but consider running the following command to give it a boost:\n' +
                        'run AutoPlay/spend-hacknet-hashes.js --spend-on Exchange_for_Bladeburner_Rank --spend-on Exchange_for_Bladeburner_SP --liquidate';
            ...
            log(ns, `WARNING: AutoPlay/bladeburner.js Caught (and suppressed) an unexpected error while waiting to join bladeburner, but will keep going:\n` +
                (typeof err === 'string' ? err : err.message || JSON.stringify(err)), false, 'warning');
        }
        await ns.sleep(5000);
    }
    ...
    const fPath = getFilePath('AutoPlay/spend-hacknet-hashes.js');
