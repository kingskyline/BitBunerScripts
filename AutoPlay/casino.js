...
export async function main(ns) {
    ...
    let abort = false;
    /*// TODO:
    // Let the user know what's going on and give them an easy way to kill AutoPlay/casino.js
    function showDialog(onCancel) {
        ...
        dlg.innerHTML = `<p>AutoPlay/casino.js is running until it wins \$10b. It will reload the save if it loses too much.<br/>` +
    ...
    async function checkForStolenFocus(throwError = true, retries = 0, silent = false) {
        ...
        let baseMessage = "It looks like something stole focus while AutoPlay/casino.js was trying to automate the casino.";
    ...
    async function checkStillAtCasino(throwError = true, silent = false) {
        ...
        let baseMessage = "It looks like the user (or another script) navigated away from the casino page" +
            " while AutoPlay/casino.js was trying to automate the casino.";
        if (throwError) // If we weren't instructed to stop whatever took focus, raise an error
            throw new Error(baseMessage + `\nPlease ensure no other scripts are running and try again ` +
    ...
    while (true) {
        ...
        try {
            ...
            if (++priorAttempts < 5) {
                ...
                log(ns, `WARNING: AutoPlay/casino.js Caught (and suppressed) an unexpected error while navigating to blackjack. ` +
                ...
                return log(ns, `ERROR: After ${priorAttempts} attempts, AutoPlay/casino.js continues to catch unexpected errors ` +
    ...
    try {
            ...
            while (winLoseTie == null) {
                ...
                try {
                    ...
                    if (midGameRetries++ >= 5)  // Retry up to 5 times before giving up and crashing out.
                        throw new Error(`After ${priorAttempts} attempts, AutoPlay/casino.js continues to catch ${errMessage}`);
                    tail(ns); // Since we're having difficulty, pop open a tail window so the user is aware and can monitor.
                    verbose = true; // Switch on verbose logs
                    log(ns, `WARNING: AutoPlay/casino.js Caught (and suppressed) ${errMessage}\n` +
        ...
        log(ns, `ERROR: AutoPlay/casino.js Caught a fatal error while playing blackjack:\n${getErrorInfo(err)}\n${supportMsg}`, true, 'error');
    }
}
...
async function reload(ns) {
    ...
    while (attempts++ <= 5) {
        ...
        errMessage = `AutoPlay/casino.js asked the game to reload ${attempts} times, but it didn't.`
...
async function onCompletion(ns, kickedOutAfterPlaying = true) {
    ...
    if (ns.run(completionScript, 1, ...completionArgs))
        log(ns, `INFO: AutoPlay/casino.js shutting down and launching ${completionScript}...`, false, 'info');
    else
        log(ns, `WARNING: AutoPlay/casino.js shutting down, but failed to launch ${completionScript}...`, false, 'warning');
}
