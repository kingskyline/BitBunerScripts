...
export async function main(ns) {
    ...
    // The name given to purchased servers (should match what's in AutoPlay/host-manager.js)
    const purchasedServersName = "server";
    ...
    async function startup(ns) {
        ...
        // Set up "asynchronous helpers" - standalone scripts to manage certain aspacts of the game. AutoPlay/daemon.js launches each of these once when ready (but not again if they are shut down)
        asynchronousHelpers = [
            { name: "/utilites/BestCustomHud.js", shouldRun: () => reqRam(64), shouldTail: false }, // Adds stats not usually in the HUD (nice to have)
            { name: "AutoPlay/go.js", shouldRun: () => reqRam(64), minRamReq: 20.2 }, // Play AutoPlay/go.js (various multipliers, but large dynamic ram requirements)
            { name: "/stocks/stockmaster.js", shouldRun: () => reqRam(64), args: openTailWindows ? ["--show-market-summary"] : [] }, // Start our stockmaster
            { name: "AutoPlay/hacknet-upgrade-manager.js", shouldRun: () => shouldUpgradeHacknet(), args: ["-c", "--max-payoff-time", "1h", "--interval", "0"], shouldTail: false }, // One-time kickstart of hash income by buying everything with up to 1h payoff time immediately
            { name: "AutoPlay/spend-hacknet-hashes.js", shouldRun: () => reqRam(64) && 9 in dictSourceFiles, args: [], shouldTail: false }, // Always have this running to make sure hashes aren't wasted
            { name: "AutoPlay/sleeve.js", shouldRun: () => reqRam(64) && 10 in dictSourceFiles }, // Script to create manage our sleeves for us
            { name: "AutoPlay/gangs.js", shouldRun: () => reqRam(64) && 2 in dictSourceFiles }, // Script to create manage our gang for us
            {
                name: "AutoPlay/work-for-factions.js", args: ['--fast-crimes-only', '--no-coding-contracts'],  // Singularity script to manage how we use our "focus" work.
            ...
            {
                name: "AutoPlay/bladeburner.js", // Script to manage bladeburner for us. Run automatically if not disabled and bladeburner API is available
                shouldRun: () => !options['disable-script'].includes('AutoPlay/bladeburner.js') && reqRam(64)
        ...
        periodicScripts = [
            ...
            { interval: 25000, name: "AutoPlay/Tasks/tor-manager.js", shouldRun: () => 4 in dictSourceFiles && !allHostNames.includes("darkweb") },
            { interval: 26000, name: "AutoPlay/Tasks/program-manager.js", shouldRun: () => 4 in dictSourceFiles && ownedCracks.length != 5 },
            { interval: 27000, name: "AutoPlay/Tasks/contractor.js", minRamReq: 14.2 }, // Periodically look for coding contracts that need solving
            ...
            { interval: 28000, name: "AutoPlay/hacknet-upgrade-manager.js", shouldRun: shouldUpgradeHacknet, args: () => ["-c", "--max-payoff-time", "4h", "--max-spend", getPlayerMoney(ns) * 0.1] },
            { interval: 28500, name: "AutoPlay/hacknet-upgrade-manager.js", shouldRun: shouldUpgradeHacknet, args: () => ["-c", "--max-payoff-time", "8h", "--max-spend", getPlayerMoney(ns) * 0.01] },
            // Buy upgrades regardless of payoff if they cost less than 0.1% of our money
            { interval: 29000, name: "AutoPlay/hacknet-upgrade-manager.js", shouldRun: shouldUpgradeHacknet, args: () => ["-c", "--max-payoff-time", "1E100h", "--max-spend", getPlayerMoney(ns) * 0.001] },
            {   // Spend about 50% of un-reserved cash on home RAM upgrades (permanent) when they become available
                interval: 30000, name: "AutoPlay/Tasks/ram-manager.js", args: () => ['--budget', 0.5, '--reserve', reservedMoney(ns)],
            ...
            {   // Periodically check for new faction invites and join if deemed useful to be in that faction. Also determines how many augs we could afford if we installed right now
                interval: 31000, name: "AutoPlay/faction-manager.js", args: ['--verbose', 'false'],
            ...
            {   // Periodically look to purchase new servers, but note that these are often not a great use of our money (hack income isn't everything) so we may hold-back.
                interval: 32000, name: "AutoPlay/host-manager.js", minRamReq: 6.55,
            ...
            { interval: 33000, name: "AutoPlay/Tasks/backdoor-all-servers.js", shouldRun: () => 4 in dictSourceFiles && playerHackSkill() > 10 }, // Don't do this until we reach hack level 10. If we backdoor too early, it's very slow and eats up RAM for a long time,
        ];
    ...
    function shouldImproveHacking() {
        return 0 != (bitNodeMults.ScriptHackMoneyGain * bitNodeMults.ScriptHackMoney) || // Check for disabled hack-income
            ...
            bitNodeN === 8 // The exception is in BN8, we still want lots of hacking to take place to manipulate stocks, which requires this infrastructure (TODO: Strike a balance between spending on this stuff and leaving money for /stocks/stockmaster.js)
    }

    /** Periodic scripts helper function: Get how much we're willing to spend on new servers (AutoPlay/host-manager.js budget) */
    ...
    async function doRoot(ns, server) {
        ...
        const pid = await exec(ns, getFilePath('AutoPlay/Tasks/crack-host.js'), daemonHost, { temporary: true }, server.name);
        ...
        do {
            ...
            try {
                ...
                if (!(4 in dictSourceFiles) && homeRam < 64) {
                    ...
                    if (loops % 600 == 0)
                        log(ns, `Reminder: AutoPlay/daemon.js can do a lot more if you have more Home RAM. Right now, you must buy this yourself.` +
                ...
                log(ns, `WARNING: AutoPlay/daemon.js Caught an error in the targeting loop: ${getErrorInfo(err)}`, true, 'warning');
                continue;
            }
        } while (!runOnce);
    }
    ...
    class Server {
        ...
        constructor(ns, node) {
            ...
            this._files = (/**@returns{Set<string>}*/() => null)(); // Unfortunately, can't cache this forever because a "AutoPlay/kill-all-scripts.js" or "AutoPlay/cleanup.js" run will wipe them.
        }
        resetCaches() {
    ...
    async function arbitraryExecution(ns, tool, threads, args, preferredServerName = null, useSmallestServerPossible = false, allowThreadSplitting = null) {
        ...
        for (let i = 0; i < rootedServersByFreeRam.length && remainingThreads > 0; i++) {
            ...
            if (targetServer.name != daemonHost && !(await tool.existsOnHost(targetServer))) {
                ...
                if (tool.name.includes("AutoPlay/Tasks/contractor.js")) // HACK: When home RAM is low and we're running this tool on another sever, copy its dependencies
    ...
    async function updateStockPositions(ns) {
        ...
        failedStockUpdates = 0;
        updatedPositions = JSON.parse(updatedPositions); // Should be a dict of stock symbol -> prob left by the /stocks/stockmaster.js script.
        ...
        while (startupAttempts++ <= 5) {
            try {
                ...
                if (startupAttempts == 5)
                    log(ns, `ERROR: AutoPlay/daemon.js Keeps catching a fatal error during startup: ${getErrorInfo(err)}`, true, 'error');
                else {
                    log(ns, `WARN: AutoPlay/daemon.js Caught an error during startup: ${getErrorInfo(err)}` +
