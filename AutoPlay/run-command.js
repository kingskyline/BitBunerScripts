...
export async function main(ns) {
    ...
    command = `ns.tprint(JSON.stringify(await (async () => ${command})() ?? "(no output)", jsonReplacer, 2)` +
        ...
        `.replaceAll(/\\n      +/gi,""))`;
    await ns.write(`AutoPlay/Temp/terminal-command.js`, "", "w"); // Clear the previous command file to avoid a warning about re-using temp script names. This is the one exception.
    return await runCommand(ns, command, `AutoPlay/Temp/terminal-command.js`, (escaped ? args.slice(1) : undefined), !silent);
}