...
export async function main(ns) {
    ...
    if (!(10 in ownedSourceFiles))
        return ns.tprint("WARNING: You cannot run AutoPlay/sleeve.js until you do BN10.");
    // Start the main loop
    while (true) {
        try { await mainLoop(ns); }
        catch (err) {
            log(ns, `WARNING: AutoPlay/sleeve.js Caught (and suppressed) an unexpected error in the main loop:\n` +
...
async function pickSleeveTask(ns, playerInfo, playerWorkInfo, i, sleeve, canTrain) {
    ...
    if (i == followPlayerSleeve && playerWorkInfo.type == "FACTION") {
        // TODO: We should be able to borrow logic from AutoPlay/work-for-factions.js to have more sleeves work for useful factions / companies
