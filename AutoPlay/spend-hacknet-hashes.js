...
export async function main(ns) {
    ...
    ns.print(`Starting AutoPlay/spend-hacknet-hashes.js... Will check in every ${formatDuration(interval)}`);
    ...
    while (true) {
        ...
        try {
            ...
            if (options['no-capacity-upgrades']) { // If we aren't allowed to purchase capacity upgrades by configuration, warn the user so they can intervene
                log(ns, `WARNING: Upgrade your hacknet cache! AutoPlay/spend-hacknet-hashes.js --no-capacity-upgrades is set, ` +
                ...
                if (nextNodeCost < nextCacheUpgradeCost && nextNodeCost < spendableMoney) {
                    if (ns.hacknet.purchaseNode())
                        log(ns, `SUCCESS: AutoPlay/spend-hacknet-hashes.js spent ${formatMoney(nextNodeCost)} to purchase a new hacknet node ${nodes + 1} ` +
                        ...
                        log(ns, `WARNING: AutoPlay/spend-hacknet-hashes.js attempted to spend ${formatMoney(nextNodeCost)} to purchase hacknet node ${nodes + 1}, `
                    ...
                    if (ns.hacknet.upgradeCache(lowestIndex, 1))
                        log(ns, `SUCCESS: AutoPlay/spend-hacknet-hashes.js spent ${formatMoney(nextCacheUpgradeCost)} to upgrade hacknet node ${lowestIndex} hash capacity ` +
                            `in order to afford further purchases (${toBuy.join(", ")}). (You can disable this with --no-capacity-upgrades)`, false, 'success');
                    else
                        log(ns, `WARNING: AutoPlay/spend-hacknet-hashes.js attempted to spend ${formatMoney(nextCacheUpgradeCost)} to upgrade hacknet node ${lowestIndex} hash capacity, `
                            `but the purchase failed for an unknown reason (despite appearing to have ${formatMoney(spendableMoney)} to spend after reserves.)`, false, 'warning');
                } else if (nodes > 0) {
                    ...
                    if (Number.isFinite(nextCheapestCacheIncreaseCost)) {
                        ...
                        log(ns, `WARNING: AutoPlay/spend-hacknet-hashes.js ${message}`, false, warnToast);
                        ...
                        log(ns, `INFO: AutoPlay/spend-hacknet-hashes.js ${message}`, true, 'info'); // Only inform the user of this the first time it happens.
            ...
            log(ns, `WARNING: AutoPlay/spend-hacknet-hashes.js Caught (and suppressed) an unexpected error in the main loop:\n` +
