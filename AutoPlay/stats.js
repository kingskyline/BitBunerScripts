...
export async function main(ns) {
    ...
    while (true) {
        try {
            ...
            log(ns, `WARNING: /utilites/BestCustomHud.js Caught (and suppressed) an unexpected error in the main loop. Update Skipped:\n` +
...
async function getHudData(ns, bitNode, dictSourceFiles, options) {
    ...
    // Show Hashes
    {
        ...
        if (9 in dictSourceFiles || 9 == bitNode) { // Section not relevant if you don't have access to hacknet servers
            const hashes = await getNsDataThroughFile(ns, '[ns.hacknet.numHashes(), ns.hacknet.hashCapacity()]', 'AutoPlay/Temp/hash-stats.txt')
            if (hashes[1] > 0) {
                ...
                const spendHashesScript = getFilePath('AutoPlay/spend-hacknet-hashes.js');
                const liquidatingHashes = await getNsDataThroughFile(ns,
                    ...
                    'AutoPlay/Temp/hash-liquidation-scripts.txt', [spendHashesScript]);
    ...
    {
        const val = ["Stock"]
        // Show Stocks (only if /stocks/stockmaster.js isn't already doing the same)
    ...
    // Show reserved money
    {
        ...
        if (reserve > 0) {
            val.push(true, formatNumberShort(reserve, 3, 2), "Most scripts will leave this much money unspent. Remove with `run AutoPlay/reserve.js 0`");
        } else val.push(false)
        hudData.push(val)
    }
    ...
    // Show current share power
    {
        ...
        if (sharePower > 1.0001) {
            val.push(true, formatNumberShort(sharePower, 3, 2),
                ...
                "\nRun `AutoPlay/daemon.js` with the `--no-share` flag to disable.");
        } else val.push(false)
        hudData.push(val)
    }
...
async function getAllServersInfo(ns) {
    ...
    return await getNsDataThroughFile(ns, 'ns.args.map(ns.getServer)', 'AutoPlay/Temp/getServers.txt', serverNames);
}
