...
let options;
const argsSchema = [
    ['l', false], // Stop any other running /stocks/stockmaster.js instances and sell all stocks
...
export async function main(ns) {
    ...
    if (!hasTixApiAccess) { // You cannot use the stockmaster until you have API access
        ...
        do {
            ...
            try {
                ...
                log(ns, `WARNING: /stocks/stockmaster.js Caught (and suppressed) an unexpected error while waiting to buy stock market access:\n` +
    ...
    while (true) {
        try {
            ...
            log(ns, `WARNING: /stocks/stockmaster.js Caught (and suppressed) an unexpected error in the main loop:\n` +
                (typeof err === 'string' ? err : err.message || JSON.stringify(err)), false, 'warning');
        }
        await ns.sleep(sleepInterval);
    }
...
function initializeHud() {
    ...
    // Remove any nested elements created by /utilites/BestCustomHud.js
