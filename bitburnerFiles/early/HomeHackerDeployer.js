...
export async function main(ns) {
  let servers = [];
  let ramPerThread = ns.getScriptRam("early/HomeHacker.js");
  ...
  while (serversToScan.length > 0) {
    ...
    if (server === "home" && !servers.includes(server)) {
      ...
      if (ns.hasRootAccess(server)) {
        await ns.scp("early/HomeHacker.js", server);
        ...
        if (usableRam > 0) {
          ...
          if (threads > 0) {
            ns.exec("early/HomeHacker.js", server, threads, target);
